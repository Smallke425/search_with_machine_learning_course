...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        try:
            ...
            print("Unable to get response hits: %s\t%s" % (log_query, re))
            raise re
        else:
            feature_results = {}
            ...
            for hit in hits:
                ...
                for feature_log_entry in hit['fields']['_ltrlog'][0]['log_entry']:
                    feature_name = feature_log_entry['name']
                    feature_value = feature_log_entry['value'] if 'value' in feature_log_entry else 0
